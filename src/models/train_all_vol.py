# src/models/train_all_vol.py
# Train Ridge per decile and produce WALK-FORWARD *volatility* predictions.
# Target: next-month squared return for each ME decile.
...
def fit_ridge_cv(X_train, y_train, max_splits: int = 5):
    ...
    # similar logic as in train_all.py
...
def walkforward_predictions(X: np.ndarray,
    ...
    """
    Same structure as in train_all.py, but now y is 'risk' (squared return).
    """
    n = len(y)
...
def train_one_file_vol(csv_path: str,
    ...
    # --- Define VOLATILITY target: next-month squared return ---
    ...
    preds_path = os.path.join(out_preds, f"{decile}_oos_vol_preds.csv")
    pd.DataFrame({
        "month": wf["months"],
    ...
    model_path = os.path.join(out_models, f"{decile}_ridge_vol.pkl")
    joblib.dump({
        "model": wf["last_model"],
    ...
    report_path = os.path.join(out_reports, f"{decile}_vol_report.txt")
...
def main():
    ...
    parser.add_argument("--models_dir", default="models_vol",
                        help="Where to save fitted VOL models.")
    parser.add_argument("--preds_dir", default="results/oos_vol_preds",
                        help="Where to save OOS VOL predictions.")
    parser.add_argument("--reports_dir", default="results/vol_reports",
                        help="Where to save VOL text reports.")
    ...
    print(f"Found {len(files)} feature files for VOL training:")
    for f in files:
        print(" -", f)
    ...
    if errors:
        ...
        with open("results/train_vol_errors.txt", "w", encoding="utf-8") as f:
            for path, msg in errors:
                f.write(f"{path}: {msg}\n")
        print("Some VOL files failed. See → results/train_vol_errors.txt")
