...
def load_feature_csv(path: str) -> pd.DataFrame:
    ...
    if ";" in head:
        ...
        # trim spaces only on string/object columns
...
def write_aligned_oos_csv(df: pd.DataFrame, path: str, max_decimals: int = 6):
    """
    Writes semicolon-separated, visually aligned CSV for OOS LR predictions.
...
def main():
    ...
    parser.add_argument("--preds_dir", default="results/oos_preds_lr",
                        help="Where to save LR OOS predictions.")
    parser.add_argument("--reports_dir", default="results/reports_lr",
