...
# Combine per-decile OOS predictions into wide and long panels for:
#   - RETURNS  (results/oos_preds/ME*_oos_preds.csv)
#   - VOLATILITY (results/oos_vol_preds/ME*_oos_vol_preds.csv)
#
# Outputs (same as before):
#   Return panels:
#       results/oos_panel/preds_panel.csv
#       results/oos_panel/true_panel.csv
#       results/oos_panel/combined_panel.csv
#       results/oos_panel/coverage.txt
#   Volatility panels:
#       results/oos_vol_panel/preds_vol_panel.csv
#       results/oos_vol_panel/true_vol_panel.csv
#       results/oos_vol_panel/coverage_vol.txt
...
RET_OUT_DIR = "results/oos_panel"
VOL_OUT_DIR = "results/oos_vol_panel"
...
# ---------- RETURN PANELS (from results/oos_preds) ----------
def build_return_panels():
    ...
    preds_path = os.path.join(RET_OUT_DIR, "preds_panel.csv")
    true_path  = os.path.join(RET_OUT_DIR, "true_panel.csv")
    ...
    combined_path = os.path.join(RET_OUT_DIR, "combined_panel.csv")
    _write_aligned_csv(combined_out, combined_path, decimals=6)

    # --- 3) Coverage report ---
    cov_path = os.path.join(RET_OUT_DIR, "coverage.txt")
...
# ---------- VOLATILITY PANELS (from results/oos_vol_preds) ----------
def build_vol_panels():
    ...
    preds_out = preds_wide.reset_index()
    true_out  = true_wide.reset_index()
    preds_path = os.path.join(VOL_OUT_DIR, "preds_vol_panel.csv")
    true_path  = os.path.join(VOL_OUT_DIR, "true_vol_panel.csv")
    _write_aligned_csv(preds_out, preds_path, decimals=6)
    _write_aligned_csv(true_out,  true_path,  decimals=6)

    # --- 2) Coverage report ---
    cov_path = os.path.join(VOL_OUT_DIR, "coverage_vol.txt")
    with open(cov_path, "w", encoding="utf-8") as f:
        f.write(f"Months: {preds_wide.shape[0]}\n")
        f.write(f"Deciles: {preds_wide.shape[1]}\n")
        f.write(f"Start: {preds_wide.index.min()}\n")
...
def main():
    # Build both return and volatility panels
    build_return_panels()
    build_vol_panels()
