...
#   - predicted volatility: results/oos_vol_panel_lr/preds_vol_panel_lr.csv (squared returns)
...
def _fmt_num(val, decimals=6):
    """Format numbers with up to `decimals` digits, without unnecessary trailing zeros."""
    if pd.isna(val):
        return ""
...
def main():
    ...
    preds_vol = load_me_panel(preds_vol_p)   # predicted "vol" (squared returns, LR vol model)
