...
def compute_turnover(weights_df: pd.DataFrame, deciles):
...
def main():
    ...
    if "month" not in W.columns or "month" not in R.columns:
        raise ValueError("Both weights_baseline.csv and true_panel_lr.csv must have a 'month' column.")
    ...
    # Build a clean weights-only DF with columns ME1..ME10 for turnover calc
